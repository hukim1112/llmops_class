...
# 관련 라이브러리 및 커스텀 모듈 가져오기 (Multimodal RAG 에이전트 연결)
...
async def run_test():
    """Multimodal RAG 파이프라인 성능을 비동기로 평가하는 메인 함수입니다."""
    
    dataset_csv_path = os.path.join(project_root, "data", "evaluation", "golden_dataset.csv")
    output_result_path = os.path.join(project_root, "data", "evaluation", "result_multimodal_rag.csv")
    
    print(f"🚀 [Multimodal RAG 평가 시작] Golden Dataset을 불러옵니다: {dataset_csv_path}")
    
    # LangSmith 트레이싱 프로젝트 이름 설정
    with tracing_v2_enabled(project_name="test_multimodal_rag"):
    ...
    print("\n==================================")
    print("📈 [결과 확인] 최종 평가 점수 산출 완료 (Multimodal RAG)")
    print("==================================")
    print(results)
